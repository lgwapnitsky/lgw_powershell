...
$sizes = foreach ($profile in $AllProfiles)
{
	...
	try 
	{
		...
		$output.LastContentModifiedDate = $profile.PersonalSite.LastContentModifiedDate
...
$sizes | select URL,PublicURL,LastContentModifiedDate,Total_B,Total_MB,Total_GB,UserName,ExistsInAD,DisabledInAD | export-csv -NoTypeInformation -Path "$pwd\MySites_Sizing.csv"
