...
$computers = Get-QADComputer -SizeLimit 0 | where {$_.accountisdisabled -eq $false}
$p_count = 0
...
foreach ($comp in $computers)
	...
	$p_count+=1
	$percentComplete = (($($p_count)/$($Computers.count))*100)
	$percentComplete = "{0:N2}" -f $percentComplete
		
	$Computer = $comp.Name
	
	write-progress -id 1 -Activity "Checking $($Computer) for logged in user: $($Username)..." -Status "$($percentComplete)% Complete" -PercentComplete $percentComplete
