IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[INT_GetGRUNDYIDCardIssuance_NR_temp]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[INT_GetGRUNDYIDCardIssuance_NR_temp]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[INT_GetGRUNDYIDCardIssuance_NR_temp]') AND type in (N'P', N'PC'))
BEGIN
...
CREATE     PROC [dbo].[INT_GetGRUNDYIDCardIssuance_NR_temp]
as
...
--and p.recordstatus not like ''%I%''
and p.transdate=''20090619''
