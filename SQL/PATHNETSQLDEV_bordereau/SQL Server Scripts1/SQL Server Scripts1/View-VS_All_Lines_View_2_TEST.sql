IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[VS_All_Lines_View_2_TEST]'))
DROP VIEW [dbo].[VS_All_Lines_View_2_TEST]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[VS_All_Lines_View_2_TEST]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.VS_All_Lines_View_2_TEST
AS
...
FROM         dbo.Latest_All_Lines_View_1_TEST
WHERE     (productcode = ''VS'') AND (Fiscal_AY_Year > 2008) AND (SubProduct = ''College / University'')
' 
GO
