IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[VS_All_Lines_View_2]'))
DROP VIEW [dbo].[VS_All_Lines_View_2]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[VS_All_Lines_View_2]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.VS_All_Lines_View_2
AS
SELECT     statecode, Coverage_Reporting_group, productcode, Fiscal_AY_Year, currentwrittenpremium, currentearnedpremium, Policy_count_Reporting_Coverage, 
                      ...
                      accountnumber, Primary_Producer_type, Playground_YN, Playground_Equip_YN, Pool_YN, Sports_Activities_YN, Tennis_Bubble_YN, Tennis_Courts_YN, 
                      ...
                      COL_Prop_Other_Count, Endorsement_Elite_YN, Incurred_Loss_ALAE_with_SS_CatOnly, Auto_Vehicle_count_Total_Range, Auto_Vehicle_count_PPT_Range, 
                      ...
                      Auto_Vehicle_count_Dual_Control_Range, Auto_Vehicle_count_Dual_Control_Percent_Range, DivisionName, producttype, Source_Detail_Name, First_Policy_Year, 
                      PIC_Producer, underwriter, Fiscal_AY_Month_Ending, Last_Reporting_Year, Last_Reporting_Month, Price_Monitor_Standard_Cov_Grp_Monitored_range, 
                      Account_UltLoss_Ratio, Account_UltLoss_Ratio_NonCat, Account_RepLoss_Ratio, Account_RepLoss_Ratio_NonCat, CompanyCode, programdesc, PICProducer, 
                      PrimaryProducer, PrimaryProducerType, Account_Premium_Range, SubProduct, SubProductGroup, BusinessClassDesc, Vocational_Schools_Avg_Rate_Range, 
                      Schools_NP_Avg_Rate_Range, Schools_FP_Avg_Rate_Range, Public_Schools_Pre_HS_Avg_Rate_Range, Public_Schools_HS_Avg_Rate_Range, 
                      Public_Schools_Avg_Rate_Range, Private_Schools_FP_Avg_Rate_Range, Private_Schools_Avg_Rate_Range, Private_Pre_HS_NP_Avg_Rate_Range, 
                      Private_Pre_HS_FP_Avg_Rate_Range, Private_Parking_Avg_Rate_Range, Private_HS_NP_Avg_Rate_Range, Dormitories_NP_Avg_Rate_Range, 
                      Dormitories_FP_Avg_Rate_Range, Dormitories_Avg_Rate_Range, College_NP_Avg_Rate_Range, College_FP_Avg_Rate_Range, accountname, effectivedate, 
                      expirationdate, PriorityAccountLevelcurrent
