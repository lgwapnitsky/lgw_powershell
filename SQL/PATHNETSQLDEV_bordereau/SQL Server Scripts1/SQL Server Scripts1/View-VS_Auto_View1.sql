IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[VS_Auto_View1]'))
DROP VIEW [dbo].[VS_Auto_View1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[VS_Auto_View1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.VS_Auto_View1
AS
...
HAVING      (dbo.Policy_Information_ALL_POLICIES_B.Primary_Umbrella = ''Primary'') AND 
                      (dbo.ALL_POLICIES_15_premium_summary_B.Coverage_Reporting_group IN (''Auto Liab'', ''Auto Phys Dam'')) AND 
                      (dbo.ALL_POLICIES_15_premium_summary_B.productcode = ''VS'')
' 
GO
