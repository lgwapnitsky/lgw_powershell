IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[INT_BOR_GetBordereauId]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[INT_BOR_GetBordereauId]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[INT_BOR_GetBordereauId]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE       Procedure [dbo].[INT_BOR_GetBordereauId] 
...
Set NoCount On
-------------------------------------------------------------------------------------------
-- Procedure [INT_BOR_GetBordereauId] - 
-- Developer:  
-- PARAMETERS:
-------------------------------------------------------------------------------------------
-- Change History:
...
--           and EffectiveDate=@EffectiveDate and isnull(generatedpolicynumber,'''')>''''
...
--	Else
		Select @BordereauId=isnull((select max(rowid) from FWA_policy     --12250
		where PolicyNumber=@PolicyNumber and EffectiveDate=@EffectiveDate),0)
End
...
IF @BordereauName=''CPH''
 Select @BordereauId=isnull((select top 1  rowid from CPH_policy 
  where PolicyNumber=@PolicyNumber and TransactionCode = 55 and EffectiveDate=@EffectiveDate),0)
