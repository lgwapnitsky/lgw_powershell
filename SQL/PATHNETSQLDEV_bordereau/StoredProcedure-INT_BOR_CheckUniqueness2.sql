IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[INT_BOR_CheckUniqueness2]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[INT_BOR_CheckUniqueness2]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[INT_BOR_CheckUniqueness2]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'/*
-------------------------------------------------------------------------------------------
-- Procedure INT_BOR_CheckUniqueness2 - 
-- Developer:  
-- PARAMETERS:
-------------------------------------------------------------------------------------------
-- Change History:
...
Create Procedure [dbo].[INT_BOR_CheckUniqueness2] 
...
--		expirationdate =@ExpirationDate and policynumber like @PolicyNumber)
--
--		SET @IsUnique=0
--	Else
--		SET @IsUnique=1
...
Begin -- FWA     --FPT12250
	...
	select @PolicyNumber = @PolicyNumber + ''%''		-- FPT 15237

	IF exists (	select	policystatus 
				from	accessphly.dbo.policy 
				where	policystatus in  (''02'',''04'') and
...
IF @BordereauName = ''CPH''
Begin -- CPH
 IF exists (SELECT * FROM CPH_policy WHERE PolicyNumber=@PolicyNumber and EffectiveDate=@EffectiveDate and TransDate=@TransmissionDate)
  SET @IsUnique=0
 else
  SET @IsUnique=1
End -- CPH
