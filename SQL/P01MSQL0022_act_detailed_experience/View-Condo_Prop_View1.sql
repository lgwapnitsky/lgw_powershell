IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[Condo_Prop_View1]'))
DROP VIEW [dbo].[Condo_Prop_View1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[Condo_Prop_View1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.Condo_Prop_View1
AS
SELECT     Coverage_Reporting_group, Coverage_Form, Fiscal_AY_Year, SUM(currentwrittenpremium) AS currentwrittenpremium, SUM(currentearnedpremium) 
                      ...
                      First_Policy_Year, Renew_New_Auto, Branch_Region, Written_Premium_Range, County_Pop_Density_Range, FA_Deductible, FA_Avg_Bld_Rate_rd2, 
                      FA_Avg_Bld_Rate_Range, FA_Constr_Code_Dominant_NAME, statecode, SUM(Earned_TIV) AS Earned_TIV, 
                      ...
                      AS Ult_LDF_Incurred_Loss_ALAE_with_SS_CatOnly, ZIP_Pop_Density_Dominant, FA_Hail_Dominant
FROM         dbo.Condo_premium_summary
GROUP BY Coverage_Reporting_group, Coverage_Form, Fiscal_AY_Year, Renewals_Range, New_Renew_Tail, First_Policy_Year, Renew_New_Auto, 
                      Branch_Region, Written_Premium_Range, County_Pop_Density_Range, FA_Deductible, FA_Avg_Bld_Rate_rd2, FA_Avg_Bld_Rate_Range, 
                      FA_Constr_Code_Dominant_NAME, statecode, ZIP_Pop_Density_Dominant, FA_Hail_Dominant
' 
GO
