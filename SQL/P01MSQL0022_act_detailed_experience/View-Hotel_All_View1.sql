IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[Hotel_All_View1]'))
DROP VIEW [dbo].[Hotel_All_View1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[Hotel_All_View1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.Hotel_All_View1
AS
SELECT     statecode, Fiscal_AY_Year, Coverage_Reporting_group, SUM(currentwrittenpremium) AS currentwrittenpremium, SUM(currentearnedpremium) 
                      ...
                      Average_Room_Rate_Range, acctngyear, Hotel_Brand, Hotel_Type, Brand_YN, Incurred_Loss_ALAE_with_SS_CatOnly, 
                      Ult_LDF_Incurred_Loss_ALAE_with_SS_CatOnly, Incurred_Loss_ALAE_with_SS_NonCat AS Expr1, 
                      Capped_Incurred_Loss_ALAE_with_SS_wo_cat AS Expr2, Ult_LDF_Incurred_Loss_ALAE_with_SS_NonCat AS Expr3, 
                      Ult_LDF_Capped_Incurred_Loss_ALAE_with_SS_wo_cat AS Expr4
...
GROUP BY statecode, Fiscal_AY_Year, Coverage_Reporting_group, Renewals_Range, New_Renew_Tail, Renew_New_Auto, First_Policy_Year, Branch_Region, 
                      ...
                      Ult_LDF_Incurred_Loss_ALAE_with_SS_NonCat, Ult_LDF_Capped_Incurred_Loss_ALAE_with_SS_wo_cat
' 
GO
