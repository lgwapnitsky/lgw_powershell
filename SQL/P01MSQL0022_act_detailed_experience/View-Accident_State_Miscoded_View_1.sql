IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[Accident_State_Miscoded_View_1]'))
DROP VIEW [dbo].[Accident_State_Miscoded_View_1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[Accident_State_Miscoded_View_1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.Accident_State_Miscoded_View_1
AS
...
HAVING      (dbo.ALL_POLICIES_15_Detail_Information_1_August.Closed_Without_Pay_Claims = 0) AND 
                      (dbo.ALL_POLICIES_15_Detail_Information_1_August.Accident_State_for_join <> dbo.ALL_POLICIES_15_Detail_Information_1_August.Premium_State) 
                      AND (dbo.ALL_POLICIES_15_Detail_Information_1_August.report_year = ''2010'')
' 
GO
