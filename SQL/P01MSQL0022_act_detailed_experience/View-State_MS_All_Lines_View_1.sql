IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[State_MS_All_Lines_View_1]'))
DROP VIEW [dbo].[State_MS_All_Lines_View_1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[State_MS_All_Lines_View_1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.State_MS_All_Lines_View_1
AS
...
GROUP BY dbo.ALL_POLICIES_12_premium_summary_May.statecode, dbo.ALL_POLICIES_12_premium_summary_May.Coverage_Reporting_group, 
                      ...
                      dbo.Policy_Information_ALL_POLICIES_AUTO_May.Hired_Auto, dbo.Policy_Information_ALL_POLICIES_AUTO_May.Non_Owned_Auto, 
                      dbo.Policy_Product_State_Coverage_Data_ALL_POLICIES_May.Price_Monitor_Standard_Cov_Grp_Monitored_range
HAVING      (dbo.ALL_POLICIES_12_premium_summary_May.statecode = ''MS'')
' 
GO
