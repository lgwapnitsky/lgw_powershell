IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[DY_Prop_View1]'))
DROP VIEW [dbo].[DY_Prop_View1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[DY_Prop_View1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.DY_Prop_View1
AS
SELECT     Coverage_Reporting_group, Coverage_Form, Fiscal_AY_Year, SUM(currentwrittenpremium) AS currentwrittenpremium, SUM(currentearnedpremium) 
                      ...
                      FA_Avg_Bld_Rate_Range, FA_Constr_Code_Dominant_NAME, FA_Avg_Bld_Rate_rd2, SUM(Earned_TIV) AS Earned_TIV, statecode, Commission_pct, 
...
GROUP BY Coverage_Reporting_group, Coverage_Form, Fiscal_AY_Year, Renewals_Range, New_Renew_Tail, Renew_New_Auto, First_Policy_Year, 
                      ...
                      FA_Avg_Bld_Rate_rd2, statecode, Commission_pct, Commission_dollars, Commission_Premiums
' 
GO
