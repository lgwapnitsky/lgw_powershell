IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[Property_All_Lines_View_State]'))
DROP VIEW [dbo].[Property_All_Lines_View_State]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[Property_All_Lines_View_State]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.Property_All_Lines_View_State
AS
SELECT     statecode, Coverage_Reporting_group, productcode, Fiscal_AY_Year, currentwrittenpremium, currentearnedpremium, Policy_count_Reporting_Coverage, 
                      ...
                      Account_RepLoss_Ratio_NonCat, CompanyCode, Endorsement_HS_004, reportmixname, Account_Premium_Range, programdesc, PICProducer, PrimaryProducer, 
                      PrimaryProducerType, SLD_BusinessType, COL_Breach_of_Contract_Count, COL_Breach_of_Contract_Loss, COL_Discrimination_Count, COL_Discrimination_Loss, 
                      ...
                      Tax_Exempt_YN, Transaction_E_Commerce_YN, Website_Transactional_E_Commerce_YN, Website_Information_Only_YN, acctngyear, FF_Cov_ALL, PV_Cov_ALL, 
                      LEFT(Insd_Zip, 3) AS Zip3
FROM         dbo.Latest_All_Lines_View_CA
WHERE     (statecode = ''CA'') AND (Fiscal_AY_Year > 2006) AND (Coverage_Reporting_group IN (''Fire & Allied''))
' 
GO
