IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[FP_Prop_View1]'))
DROP VIEW [dbo].[FP_Prop_View1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[FP_Prop_View1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.FP_Prop_View1
AS
SELECT     Coverage_Reporting_group, Fiscal_AY_Month_Ending, Fiscal_AY_Year, SUM(currentearnedpremium) AS currentearnedpremium, 
                      ...
                      County_Pop_Density_Range, FA_Avg_Bld_Rate_rd2, FA_Constr_Code_Dominant_NAME, Coverage_Form, statecode, Companycode, 
                      GL_Dom_Region_in_State, SUM(Earned_TIV) AS Earned_TIV, FA_protect_class_Dominant
...
GROUP BY Coverage_Reporting_group, Fiscal_AY_Month_Ending, Fiscal_AY_Year, New_Renew_Tail, Renew_New_Auto, Written_Premium_Range, 
                      ...
                      FA_Constr_Code_Dominant_NAME, Coverage_Form, statecode, Companycode, GL_Dom_Region_in_State, FA_protect_class_Dominant
' 
GO
