IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[usp_Step_1_Premium]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[usp_Step_1_Premium]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[usp_Step_1_Premium]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'
CREATE procedure [dbo].[usp_Step_1_Premium]
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N''[dbo].[ALL_POLICIES_12_premium_summary_work]'') AND type in (N''U''))
Drop Table act_detailed_experience.dbo.ALL_POLICIES_12_premium_summary_work;

CREATE TABLE [dbo].[ALL_POLICIES_12_premium_summary_work](
...
Set @First_Year = (select First_Year from Detailed_Experience_Parameters)  /*should be one year prior to the first year displayed
...
Set @Last_Reporting_Year = (select Last_Reporting_Year from Detailed_Experience_Parameters) 	--must be four digits; and must match loss evaluation year

declare @Last_Reporting_Month as int
Set @Last_Reporting_Month = (select Last_Reporting_Month from Detailed_Experience_Parameters)	-- must match loss evaluation month
...
update act_detailed_experience.dbo.ALL_POLICIES_12_premium_summary_work
set 
policyeffectivedate = (
