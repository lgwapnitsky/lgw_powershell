IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[LDF]') AND type in (N'U'))
DROP TABLE [dbo].[LDF]
GO
SET ANSI_NULLS ON
GO
...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[LDF]') AND type in (N'U'))
BEGIN
CREATE TABLE [dbo].[LDF](
