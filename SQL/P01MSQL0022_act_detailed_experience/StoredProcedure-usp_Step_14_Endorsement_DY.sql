IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[usp_Step_14_Endorsement_DY]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[usp_Step_14_Endorsement_DY]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[usp_Step_14_Endorsement_DY]') AND type in (N'P', N'PC'))
BEGIN
...
CREATE Procedure [dbo].[usp_Step_14_Endorsement_DY] as
...
Endorsement_DY_CG2504 = ISNULL( 
		...
		WHERE l.nexpnum = 0 
			...
			and l.dat_bdtid IN (
				...
				where BD2.bdtNumber LIKE ''%CG%2504%'')
		),
		-- V2 
		...
		from PICPFN01.aqs.MSSNM1POLFMB as forms
	        where forms.KSTOKEY = PS.PolicyID
		       and forms.NEXPNUM = 0
		       and forms.LFRMNUM LIKE ''CG%2504%'')
	))
