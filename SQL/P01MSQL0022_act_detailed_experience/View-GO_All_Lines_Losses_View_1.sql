IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[GO_All_Lines_Losses_View_1]'))
DROP VIEW [dbo].[GO_All_Lines_Losses_View_1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[GO_All_Lines_Losses_View_1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.GO_All_Lines_Losses_View_1
AS
SELECT     v1.accountnumber, v1.Policy_Number, v1.Claim_Number, pol.Productcode, v1.Coverage_Reporting_group, v1.Coverage_Form, v1.Accident_Year, 
...
FROM         dbo.Latest_All_Lines_Losses_View_1 AS v1 LEFT OUTER JOIN
                      dbo.Policy_Information_ALL_POLICIES_February AS pol ON v1.Policy_Number = pol.policynumber LEFT OUTER JOIN
                      AccessPhlyEom.dbo.claimsummary AS cs ON v1.Claim_Number = cs.claimnumber
WHERE     (v1.Fiscal_AY_Year >= 2006) AND (pol.Productcode = ''go'')
' 
GO
