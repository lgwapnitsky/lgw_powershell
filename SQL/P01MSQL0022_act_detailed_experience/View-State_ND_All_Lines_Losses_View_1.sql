IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[State_ND_All_Lines_Losses_View_1]'))
DROP VIEW [dbo].[State_ND_All_Lines_Losses_View_1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[State_ND_All_Lines_Losses_View_1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.State_ND_All_Lines_Losses_View_1
AS
...
WHERE     (dbo.ALL_POLICIES_12_Detail_Information_1_November.Premium_State = ''ND'')
' 
GO
