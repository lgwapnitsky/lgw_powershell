IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[State_ME_All_Lines_View_1]'))
DROP VIEW [dbo].[State_ME_All_Lines_View_1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[State_ME_All_Lines_View_1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.State_ME_All_Lines_View_1
AS
...
GROUP BY dbo.ALL_POLICIES_12_premium_summary_September.statecode, 
                      ...
                      dbo.Policy_Information_ALL_POLICIES_September.BusinessClassDesc, 
                      dbo.Policy_Information_ALL_POLICIES_September.Source_Detail_Name
HAVING      (dbo.ALL_POLICIES_12_premium_summary_September.statecode = ''ME'')
' 
GO
