IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[usp_Step_17_Premium_15_Month]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[usp_Step_17_Premium_15_Month]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[usp_Step_17_Premium_15_Month]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'
CREATE procedure [dbo].[usp_Step_17_Premium_15_Month]

as

-- Revision #1
-- FootPrints Ticket #69522
-- Revisor:	Erik Godfrey
-- Revision Date: 3/12/2013
-- Revision Description: Change table for Policy_count_All_Cov_Combined assignment
...
/* Step 1 */
/* Create Table structure for premium file */
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N''[dbo].[ALL_POLICIES_15_premium_summary_work]'') AND type in (N''U''))
Drop Table act_detailed_experience.dbo.ALL_POLICIES_15_premium_summary_work;

CREATE TABLE [dbo].[ALL_POLICIES_15_premium_summary_work](
...
-- 2/5/2010 took 10 hrs

declare @First_Year as int
Set @First_Year = (select First_Year from dbo.Detailed_Experience_Parameters)
						/*should be one year prior to the first year displayed
						in the exhibits, so as to capture all writings from that year
						that earn in the first displayed year*/

declare @Last_Reporting_Year as int
Set @Last_Reporting_Year = (select Last_Reporting_Year from dbo.Detailed_Experience_Parameters)	--must be four digits; and must match loss evaluation year

declare @Last_Reporting_Month as int
Set @Last_Reporting_Month = (select Last_Reporting_Month from dbo.Detailed_Experience_Parameters)	-- must match loss evaluation month

declare @Fiscal_AY_Month_Ending as int
Set @Fiscal_AY_Month_Ending = (select Fiscal_AY_Month_Ending_15_Month from dbo.Detailed_Experience_Parameters)


insert into act_detailed_experience.dbo.ALL_POLICIES_15_premium_summary_work
(	accountnumber,
	policynumber,
	policyeffectivedate,
	statecode,
...
/*	Coverage_id = (select distinct CG.Coverage_id
			  from Act_Definitions.dbo.Coverage_Groups CG
			  where CG.Coveragecode = ps.Coveragecode),
*/
	(case 
...
''YU'')
	...
	(CG.Prop_Indication_key) Coverage_Form,
	ps.productcode,
...
/*
		...
		when @Fiscal_AY_Month_Ending = 1 then 
			...
			where PS.Acctngyear = YR.Calendar_Year
				and PS.acctngmonth = YR.Calendar_Month)
...
--  and productcode in (''me'')
--	and companycode = ''PH''
--	and statecode = ''MD''
--	and coveragecode in (''GLBI'',''PLGL'',''GLDX'',''SAM'')
--	and aslobcode in (''170'') 
--	and productcode not in (''AE'',''CL'',''LE'',''HP'',''FF'',''DF'',''PV'',''CD'',''EX'',''EL'',''EW'',''ER'')
--	and aslobcode in (''10'',''20'',''21'',''90'',''120'',''170'',''193'',''194'',''212'',''230'',''250'',''260'',''270'') 
/*
	and productcode in (
''AC'',
''AM'',
...
*/
/*
-- Update Umbrella product codes
select distinct char(39)+productcode+char(39)+'','' from Accessphlyeom.dbo.ExperienceProducts
where UnderwritingLOB = ''UMB''
*/
) as step1
...
--------------------------------------------------------------------------
/* Step 2 */
/* Add Policy Count fields to PremiumSummary table */
...
Policy_count_All_Cov_Combined = (case 
	when PS.currentearnedpremium is not null 
		...
		and All_Cov.Currentwrittenpremium > 0  -- Revision #1
	then convert(decimal(20,8),currentearnedpremium) / All_Cov.Currentwrittenpremium  -- Revision #1
	else NULL end),
