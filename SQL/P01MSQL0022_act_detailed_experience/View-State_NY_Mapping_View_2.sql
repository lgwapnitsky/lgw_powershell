IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[State_NY_Mapping_View_2]'))
DROP VIEW [dbo].[State_NY_Mapping_View_2]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[State_NY_Mapping_View_2]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.State_NY_Mapping_View_2
AS
...
FROM         Act_Exposure_Mgt.dbo.PropertyUpdate_General_2012_11_30 AS pgt LEFT OUTER JOIN
                      dbo.Policy_PATHID_Information AS path ON pgt.Account_Number = path.accountnumber
WHERE     (pgt.State = ''NY'') AND (pgt.Policy_Number = path.policynumber)
' 
GO
