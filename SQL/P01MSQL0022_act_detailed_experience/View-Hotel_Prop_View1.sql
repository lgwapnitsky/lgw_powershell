IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[Hotel_Prop_View1]'))
DROP VIEW [dbo].[Hotel_Prop_View1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[Hotel_Prop_View1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.Hotel_Prop_View1
AS
SELECT     Coverage_Reporting_group, Coverage_Form, Fiscal_AY_Year, Policy_count_Reporting_Coverage, SUM(currentearnedpremium) 
                      ...
                      Incurred_Loss_ALAE_with_SS_CatOnly, Ult_LDF_Incurred_Loss_ALAE_with_SS_CatOnly, Incurred_Loss_ALAE_with_SS_NonCat AS Expr1, 
                      Capped_Incurred_Loss_ALAE_with_SS_wo_cat AS Expr2, Ult_LDF_Capped_Incurred_Loss_ALAE_with_SS_wo_cat AS Expr3, 
                      Ult_LDF_Incurred_Loss_ALAE_with_SS_NonCat AS Expr4
FROM         dbo.Hotel_Premium_Summary
GROUP BY Coverage_Reporting_group, Coverage_Form, Fiscal_AY_Year, Policy_count_Reporting_Coverage, Renewals_Range, New_Renew_Tail, 
                      Renew_New_Auto, Branch_Region, First_Policy_Year, Written_Premium_Range, County_Pop_Density_Range, FA_Deductible, FA_Avg_Bld_Rate_rd2, 
                      FA_Avg_Bld_Rate_Range, FA_Constr_Code_Dominant_NAME, Number_of_Rooms_Range, Incurred_Loss_ALAE_with_SS_CatOnly, 
                      Ult_LDF_Incurred_Loss_ALAE_with_SS_CatOnly, Incurred_Loss_ALAE_with_SS_NonCat, Capped_Incurred_Loss_ALAE_with_SS_wo_cat, 
                      Ult_LDF_Capped_Incurred_Loss_ALAE_with_SS_wo_cat, Ult_LDF_Incurred_Loss_ALAE_with_SS_NonCat
' 
GO
