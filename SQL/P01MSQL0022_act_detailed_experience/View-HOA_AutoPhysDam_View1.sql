IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[HOA_AutoPhysDam_View1]'))
DROP VIEW [dbo].[HOA_AutoPhysDam_View1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[HOA_AutoPhysDam_View1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW [dbo].[HOA_AutoPhysDam_View1]
AS
SELECT     Coverage_Reporting_group, Fiscal_AY_Year, SUM(currentwrittenpremium) AS currentwrittenpremium, SUM(currentearnedpremium) 
                      ...
                      Auto_Vehicle_count_PPT_Range, Auto_Vehicle_count_Truck_Range, Auto_Vehicle_count_Total_Range
