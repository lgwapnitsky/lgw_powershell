IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[ME_GL_View1]'))
DROP VIEW [dbo].[ME_GL_View1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[ME_GL_View1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.ME_GL_View1
AS
SELECT     Fiscal_AY_Year, SUM(Policy_count_Reporting_Coverage) AS Policy_count_Reporting_Coverage, SUM(currentearnedpremium) 
                      ...
                      AS Reported_Claims, New_Renew_Tail, GL_Class_AlcoholDrug_AvgRate_Range, GL_Class_HomesForHandicapped_AvgRate_Range, 
                      GL_Class_PsychopathicInstitutions_AvgRate_Range, GL_Class_OutpatientClinics_AvgRate_Range, GL_Class_BoardingHalfwayHouses_AvgRate, 
                      GL_Class_Offices_AvgRate_Range, GL_Class_AlcoholDrug_AvgRate, GL_Class_HomesForHandicapped_AvgRate, 
                      GL_Class_PsychopathicInstitutions_AvgRate, GL_Class_OutpatientClinics_AvgRate, COL_GLBI_Malpractice_Count, COL_GLBI_Malpractice_Loss, 
                      ...
                      COL_GLBI_Discrim_WrongfulTermination_Count, Renewals_Range, Renew_New_Auto, First_Policy_Year, County_Pop_Density_Range, 
                      Price_Monitor_Renewal_Changes_of_CovGrp_St_Class_Monitored_range, GL_Deductible, GL_Occurrence_Limit, Written_Premium_Range, 
                      GL_Class_BoardingHalfwayHouses_AvgRate_Range, GL_Class_Offices_AvgRate, Coverage_Reporting_group, GLDX_Indicator, 
                      Price_Monitor_Standard_Cov_Grp_GL_Monitored_range
FROM         dbo.mental_health_premium_summary
GROUP BY Fiscal_AY_Year, New_Renew_Tail, GL_Class_AlcoholDrug_AvgRate_Range, GL_Class_HomesForHandicapped_AvgRate_Range, 
                      GL_Class_PsychopathicInstitutions_AvgRate_Range, GL_Class_OutpatientClinics_AvgRate_Range, GL_Class_BoardingHalfwayHouses_AvgRate, 
                      GL_Class_Offices_AvgRate_Range, GL_Class_AlcoholDrug_AvgRate, GL_Class_HomesForHandicapped_AvgRate, 
                      GL_Class_PsychopathicInstitutions_AvgRate, GL_Class_OutpatientClinics_AvgRate, COL_GLBI_Malpractice_Count, COL_GLBI_Malpractice_Loss, 
                      COL_GLBI_Fraud_Breach_of_Contract_Count, COL_GLBI_Fraud_Breach_of_Contract_Loss, COL_GLBI_Injury_Count, COL_GLBI_Injury_Loss, 
