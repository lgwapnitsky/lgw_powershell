IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[NP_AutoLiab_View1]'))
DROP VIEW [dbo].[NP_AutoLiab_View1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[NP_AutoLiab_View1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.NP_AutoLiab_View1
AS
SELECT     Fiscal_AY_Year, Written_Premium_Range, SUM(Policy_count_Reporting_Coverage) AS Policy_count_Reporting_Coverage, 
                      ...
                      Latest_Product, GL_Dom_Region_in_State
...
GROUP BY Fiscal_AY_Year, Written_Premium_Range, COL_AUTO_LIAB_AllOther_Count, COL_AUTO_LIAB_AllOther_Loss, COL_AUTO_LIAB_PropDam_Count, 
                      ...
                      Auto_Vehicle_count_Total_Range, Coverage_Reporting_group, Companycode, Latest_Product, GL_Dom_Region_in_State
' 
GO
