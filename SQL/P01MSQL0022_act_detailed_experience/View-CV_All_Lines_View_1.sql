IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[CV_All_Lines_View_1]'))
DROP VIEW [dbo].[CV_All_Lines_View_1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[CV_All_Lines_View_1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.CV_All_Lines_View_1
AS
SELECT     statecode, Coverage_Reporting_group, productcode, Fiscal_AY_Year, currentwrittenpremium, currentearnedpremium, Policy_count_Reporting_Coverage, 
                      ...
                      accountnumber, Primary_Producer_type, Incurred_Loss_ALAE_with_SS, Capped_Incurred_Loss_ALAE_with_SS, Ult_LDF_Incurred_Loss_ALAE_with_SS, 
                      ...
                      Account_RepLoss_Ratio_NonCat, programdesc, PICProducer, PrimaryProducer, PrimaryProducerType, Account_Premium_Range
FROM         dbo.Latest_All_Lines_View_1
WHERE     (productcode = ''CV'') AND (Fiscal_AY_Year > 2007)
' 
GO
