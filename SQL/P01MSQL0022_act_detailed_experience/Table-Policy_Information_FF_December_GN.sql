IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Policy_Information_FF_December_GN]') AND type in (N'U'))
DROP TABLE [dbo].[Policy_Information_FF_December_GN]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Policy_Information_FF_December_GN]') AND type in (N'U'))
BEGIN
CREATE TABLE [dbo].[Policy_Information_FF_December_GN](
	...
	[FF_EPL_Num_of_Employees_Range_2] [varchar](50) COLLATE SQL_Latin1_General_CP1_CI_AS NULL
) ON [PRIMARY]
END
GO
