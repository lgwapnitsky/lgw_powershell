IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[usp_Step_6b_Producer_Info_Premium]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[usp_Step_6b_Producer_Info_Premium]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[usp_Step_6b_Producer_Info_Premium]') AND type in (N'P', N'PC'))
BEGIN
...
CREATE procedure [dbo].[usp_Step_6b_Producer_Info_Premium] as
...
select [accountnumber]
      ...
      ,pol.Commission_pct
      ,(case when pol.Commission_pct is not null then currentwrittenpremium else 0.0 end) as Commission_Premiums
      ,pol.Commission_pct * currentwrittenpremium as Commission_dollars
...
left outer join (select distinct policynumber, Commission_pct from
Act_Detailed_Experience.dbo.Policy_Information_ALL_POLICIES_work) POL on 
POL.policynumber = PS.policynumber
