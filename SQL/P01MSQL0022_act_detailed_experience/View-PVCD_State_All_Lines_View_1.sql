IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[PVCD_State_All_Lines_View_1]'))
DROP VIEW [dbo].[PVCD_State_All_Lines_View_1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[PVCD_State_All_Lines_View_1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.PVCD_State_All_Lines_View_1
AS
SELECT     statecode, Coverage_Reporting_group, productcode, Fiscal_AY_Year, currentwrittenpremium, currentearnedpremium, 
                      ...
                      First_Policy_Year, Clean_Warranty_Letter_Received_YN, Price_Monitor_Standard_Cov_Grp_Monitored_range, COL_DO_Other_Count, 
                      COL_DO_Other_Loss, COL_EPL_Other_Count, COL_EPL_Other_Loss, COL_Fiduciary_Other_Count, COL_Fiduciary_Other_Loss, 
                      COL_Worplace_Violence_Other_Count, COL_Worplace_Violence_Other_Loss, COL_Internet_Other_Count, COL_Internet_Other_Loss, 
                      PVCD_PVDO_ZoneMod, PVCD_PVEPL_ZoneMod, PVCD_PVEPL_Num_of_Employees, Fiscal_AY_Month_Ending, Last_Reporting_Year, 
                      Last_Reporting_Month, underwriter, PIC_Producer, Source_Detail_Name, BusinessClassDesc, ProductType, DivisionName, 
                      Written_Premium_Range_for_Automatics, PVCD_ZoneNum_EPL, PVCD_ZoneNum_DO, PV_Zone_Factor_EPL, PV_Zone_Factor_DO, 
                      PVCD_ZipCode_DO, PVCD_ZipCode_EPL, PVCD_State_ZipCode_EPL, PVCD_State_ZipCode_DO, PVCD_State_DO, PVCD_State_EPL, 
                      PVCD_State_Zip3_DO, PVCD_State_Zip3_EPL, PVCD_County_DO, PVCD_County_EPL, PVCD_County_EPL_DO
FROM         dbo.PVCD_All_Lines_View_1
WHERE     (statecode = ''TX'')
' 
GO
