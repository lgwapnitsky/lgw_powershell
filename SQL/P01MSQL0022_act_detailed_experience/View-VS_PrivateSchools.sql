IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[VS_PrivateSchools]'))
DROP VIEW [dbo].[VS_PrivateSchools]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[VS_PrivateSchools]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.VS_PrivateSchools
AS
SELECT     dbo.VS_All_Lines_View_2.statecode, dbo.VS_All_Lines_View_2.Coverage_Reporting_group, dbo.VS_All_Lines_View_2.productcode, 
                      ...
                      dbo.Latest_Policy_Information_UWChecklist_View_1_table.Turnover_Rate_Range
...
WHERE     (dbo.VS_All_Lines_View_2.SubProduct = ''Private Schools'')
' 
GO
