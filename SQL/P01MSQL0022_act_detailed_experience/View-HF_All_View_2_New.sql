IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[HF_All_View_2_New]'))
DROP VIEW [dbo].[HF_All_View_2_New]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[HF_All_View_2_New]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.HF_All_View_2_New
AS
...
HAVING      (dbo.Policy_Information_ALL_POLICIES_August.Primary_Umbrella = ''Primary'') AND 
                      (dbo.ALL_POLICIES_15_premium_summary_August.productcode = ''HF'')
' 
GO
