IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[All_Policies_All_LOB_15mon_tiein_View_October]'))
DROP VIEW [dbo].[All_Policies_All_LOB_15mon_tiein_View_October]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[All_Policies_All_LOB_15mon_tiein_View_October]'))
EXEC dbo.sp_executesql @statement = N'

CREATE VIEW [dbo].[All_Policies_All_LOB_15mon_tiein_View_October]
AS
...
FROM         dbo.ALL_POLICIES_15_premium_summary_work
