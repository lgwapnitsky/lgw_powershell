IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[PVCD_wo_CA_TopClaims_View_1]'))
DROP VIEW [dbo].[PVCD_wo_CA_TopClaims_View_1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[PVCD_wo_CA_TopClaims_View_1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW [dbo].[PVCD_wo_CA_TopClaims_View_1]
AS
...
HAVING      (Product = ''PV'') AND (Accident_State_for_join <> ''ca'')
' 
GO
