IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[State_NY_All_Lines_View_2]'))
DROP VIEW [dbo].[State_NY_All_Lines_View_2]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[State_NY_All_Lines_View_2]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.State_NY_All_Lines_View_2
AS
SELECT     statecode, Coverage_Reporting_group, productcode, Fiscal_AY_Year, currentwrittenpremium, currentearnedpremium, Policy_count_Reporting_Coverage, 
                      ...
                      Auto_Vehicle_count_Truck_Range, Auto_Vehicle_count_PPT_Range, Auto_Vehicle_count_Buses_Range, Auto_Vehicle_count_VanPools_Range, 
                      Auto_Vehicle_count_OtherPublic_Range, Auto_Vehicle_count_Other_Range, Auto_Vehicle_count_Dual_Control_Range, 
                      Auto_Vehicle_count_Dual_Control_Percent_Range, Hired_Auto, Non_Owned_Auto, Churches_Avg_Rate_Range, Day_Care_Avg_Rate_Range, Primary_Producer_code, 
                      Account_RepLoss_Ratio_NonCat_Working_Cover_priorpolperiods_Range, Account_UltLoss_Ratio_NonCat_Working_Cover_priorpolperiods_Range, 
                      Account_UltLoss_Ratio_Working_Cover_priorpolperiods_Range, Account_RepLoss_Ratio_Working_Cover_priorpolperiods_Range, 
                      Account_RepLoss_Ratio_NonCat_priorpolperiods_Range, Account_UltLoss_Ratio_NonCat_priorpolperiods_Range, Account_RepLoss_Ratio_priorpolperiods_Range, 
                      Account_UltLoss_Ratio_priorpolperiods_Range, Account_RepLoss_Ratio_NonCat_Range, Account_UltLoss_Ratio_NonCat_Working_Cover_Range, 
                      Account_RepLoss_Ratio_NonCat_Working_Cover_Range, Price_Monitor_Standard_Cov_Grp_Monitored_range, DivisionName, producttype, Fiscal_AY_Month_Ending, 
                      Last_Reporting_Year, Last_Reporting_Month, PIC_Producer, First_Policy_Year, underwriter, BusinessClassDesc, Source_Detail_Name
FROM         dbo.State_Latest_All_Lines_View_1
WHERE     (statecode = ''NY'')
' 
GO
