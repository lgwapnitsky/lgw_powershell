IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[Fire_&_Allied_All_Lines_View_CA]'))
DROP VIEW [dbo].[Fire_&_Allied_All_Lines_View_CA]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[Fire_&_Allied_All_Lines_View_CA]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW [dbo].[Fire_&_Allied_All_Lines_View_CA]
AS
SELECT     dbo.Latest_All_Lines_View_CA.statecode, dbo.Latest_All_Lines_View_CA.Coverage_Reporting_group, dbo.Latest_All_Lines_View_CA.productcode, 
                      ...
                      dbo.Latest_All_Lines_View_CA.PV_Cov_ALL, LEFT(dbo.Latest_All_Lines_View_CA.Insd_Zip, 3) AS Zip3, 
                      dbo.Policy_Product_State_Coverage_Data_ALL_POLICIES_December_test.FA_Territory_Dominant, 
                      dbo.Policy_Product_State_Coverage_Data_ALL_POLICIES_December_test.FA_Territory_Dominant_percent, 
                      dbo.Policy_Product_State_Coverage_Data_ALL_POLICIES_December_test.FA_Dom_Terr_description, 
                      dbo.Policy_Product_State_Coverage_Data_ALL_POLICIES_December_test.FA_Dom_Region_in_State
...
WHERE     (dbo.Latest_All_Lines_View_CA.statecode = ''CA'') AND (dbo.Latest_All_Lines_View_CA.Fiscal_AY_Year > 2006) AND 
                      (dbo.Latest_All_Lines_View_CA.Coverage_Reporting_group IN (''Fire & Allied''))
' 
GO
