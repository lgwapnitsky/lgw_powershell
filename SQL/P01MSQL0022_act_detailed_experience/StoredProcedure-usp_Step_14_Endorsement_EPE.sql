IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[usp_Step_14_Endorsement_EPE]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[usp_Step_14_Endorsement_EPE]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[usp_Step_14_Endorsement_EPE]') AND type in (N'P', N'PC'))
BEGIN
...
CREATE procedure [dbo].[usp_Step_14_Endorsement_EPE] as
...
Endorsement_FA_Elite = ISNULL( 
		...
		WHERE l.nexpnum = 0 
			...
			and l.dat_bdtid IN (
				...
				where BD2.bdtNumber LIKE ''%PI-EPE%'')
		),   --'''')
		-- V2 
		((select distinct top 1 forms.lfrmnum
		from PICPFN01.aqs.MSSNM1POLFMB as forms
	        where forms.KSTOKEY = PS.PolicyID
		       and forms.NEXPNUM = 0
		       and forms.LFRMNUM LIKE ''%PIEPE%'')
	))
from Act_Detailed_Experience.dbo.Policy_Information_ALL_POLICIES_work  PS

update Act_Detailed_Experience.dbo.Policy_Information_ALL_POLICIES_work 
set 
Endorsement_Elite_YN = (case
	when Endorsement_FA_Elite is not null then ''Y'' else ''N'' end)
