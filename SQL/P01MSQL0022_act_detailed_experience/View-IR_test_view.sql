IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[IR_test_view]'))
DROP VIEW [dbo].[IR_test_view]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[IR_test_view]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.IR_test_view
AS
SELECT     statecode, Coverage_Reporting_group, productcode, Fiscal_AY_Year, currentwrittenpremium, currentearnedpremium, Policy_count_Reporting_Coverage, 
                      ...
                      PrimaryProducer, PrimaryProducerType, Account_Premium_Range, effectivedate, expirationdate
FROM         dbo.Latest_All_Lines_View_1
WHERE     (productcode IN (''IR'', ''IM'')) AND (Fiscal_AY_Year > 2008)
' 
GO
