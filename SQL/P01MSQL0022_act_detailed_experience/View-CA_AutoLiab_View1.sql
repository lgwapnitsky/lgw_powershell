IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[CA_AutoLiab_View1]'))
DROP VIEW [dbo].[CA_AutoLiab_View1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[CA_AutoLiab_View1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.CA_AutoLiab_View1
AS
SELECT     Fiscal_AY_Year, SUM(Policy_count_Reporting_Coverage) AS Policy_count_Reporting_Coverage, SUM(currentearnedpremium) 
                      ...
                      AS COL_AUTO_LIAB_PropDam_Loss, Coverage_Reporting_group
