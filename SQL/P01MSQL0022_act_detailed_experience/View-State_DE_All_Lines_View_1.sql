IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[State_DE_All_Lines_View_1]'))
DROP VIEW [dbo].[State_DE_All_Lines_View_1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[State_DE_All_Lines_View_1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.State_DE_All_Lines_View_1
AS
...
GROUP BY dbo.ALL_POLICIES_12_premium_summary_June.statecode, dbo.ALL_POLICIES_12_premium_summary_June.Coverage_Reporting_group, 
                      ...
                      dbo.Policy_Product_State_Coverage_Data_ALL_POLICIES_June.Price_Monitor_Standard_Cov_Grp_Monitored_range, 
                      AccessPhlyEom.dbo.product.producttype, AccessPhlyEom.dbo.Division.DivisionName
HAVING      (dbo.ALL_POLICIES_12_premium_summary_June.statecode = ''DE'') AND (NOT (dbo.ALL_POLICIES_12_premium_summary_June.productcode IN (''NP'', 
                      ''CO'')))
' 
GO
