IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[All_Policies_ALL_LOB_12mon_tiein_View_August]'))
DROP VIEW [dbo].[All_Policies_ALL_LOB_12mon_tiein_View_August]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[All_Policies_ALL_LOB_12mon_tiein_View_August]'))
EXEC dbo.sp_executesql @statement = N'



CREATE VIEW [dbo].[All_Policies_ALL_LOB_12mon_tiein_View_August]
AS
...
FROM         dbo.ALL_POLICIES_12_premium_summary_August
