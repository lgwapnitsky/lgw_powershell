IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[AK_All_Lines_View_1]'))
DROP VIEW [dbo].[AK_All_Lines_View_1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[AK_All_Lines_View_1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.AK_All_Lines_View_1
AS
SELECT     statecode, Coverage_Reporting_group, Fiscal_AY_Year, productcode, currentwrittenpremium, currentearnedpremium, Policy_count_Reporting_Coverage, 
...
WHERE     (productcode = ''AK'') AND (Fiscal_AY_Year > 2007)
' 
GO
