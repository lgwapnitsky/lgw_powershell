IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[Auto_All_Lines_View_1]'))
DROP VIEW [dbo].[Auto_All_Lines_View_1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[Auto_All_Lines_View_1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.Auto_All_Lines_View_1
AS
SELECT     statecode, Coverage_Reporting_group, productcode, Fiscal_AY_Year, currentwrittenpremium, currentearnedpremium, Policy_count_Reporting_Coverage, 
                      ...
                      Endorsement_HS_003, Endorsement_HS_003D, Written_Premium_Range
FROM         dbo.Latest_All_Lines_View_1
WHERE     (Fiscal_AY_Year > 2008) AND (Coverage_Reporting_group IN (''Auto Liability'', ''Auto Phys Dam''))
' 
GO
