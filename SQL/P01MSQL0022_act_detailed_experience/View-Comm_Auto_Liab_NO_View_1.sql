IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[Comm_Auto_Liab_NO_View_1]'))
DROP VIEW [dbo].[Comm_Auto_Liab_NO_View_1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[Comm_Auto_Liab_NO_View_1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.Comm_Auto_Liab_NO_View_1
AS
...
FROM         dbo.Comm_Auto_Liab_NO_premium_summary
