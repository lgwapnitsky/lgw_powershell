IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[Crime_View_2]'))
DROP VIEW [dbo].[Crime_View_2]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[Crime_View_2]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.Crime_View_2
AS
...
WHERE     (dbo.Latest_All_Lines_View_1.Fiscal_AY_Year > 2008) AND (dbo.Latest_All_Lines_View_1.productcode IN (''CL'', ''FF'', ''DF'', ''PV'', ''AE'')) AND 
                      (dbo.Latest_All_Lines_View_1.Coverage_Group IN (''Crime'', ''Fidelity'', ''BOP CR''))
' 
GO
