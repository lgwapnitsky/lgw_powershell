IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[GENERAL_LIABILITY_View_1]'))
DROP VIEW [dbo].[GENERAL_LIABILITY_View_1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[GENERAL_LIABILITY_View_1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.GENERAL_LIABILITY_View_1
AS
SELECT     statecode, Coverage_Reporting_group, productcode, Fiscal_AY_Year, currentwrittenpremium, currentearnedpremium, Policy_count_Reporting_Coverage, 
                      ...
                      Written_Premium_Range, Number_of_Renewals, PICProducer, PrimaryProducer, PrimaryProducerType, Account_Premium_Range, DivisionName, producttype, 
                      PrimaryProducerNumber, effectivedate, expirationdate, Insured_Years_Range, SLD_BusinessType, Incurred_Loss_ALAE_without_SS_NonCat, accountnumber, 
                      Source_Detail_Name, BusinessClassDesc, COL_Breach_of_Contract_Count, COL_Breach_of_Contract_Loss, COL_Discrimination_Count, COL_Discrimination_Loss, 
                      ...
                      Expr1, Endorsement_HS_003, Endorsement_HS_003D
FROM         dbo.Latest_All_Lines_View_1
WHERE     (statecode = ''PA'') AND (Coverage_Group = ''gl'') AND (Fiscal_AY_Year > 2007)
' 
GO
