IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[HF_ANGOSS_View_1]'))
DROP VIEW [dbo].[HF_ANGOSS_View_1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[HF_ANGOSS_View_1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.HF_ANGOSS_View_1
AS
SELECT     Coverage_Reporting_group, Fiscal_AY_Year, Policy_count_Reporting_Coverage, SUM(COL_GLBI_Malpractice_Count) 
                      ...
                      Renewals_Range, New_Renew_Tail, Renew_New_Auto, First_Policy_Year, County_Pop_Density_Range, 
                      ...
                      AS Ult_LDF_Incurred_Loss_ALAE_with_SS_NonCat, SUM(currentearnedpremium) AS currentearnedpremium, GLDX_Indicator, Athletic_Participants, 
                      Daycare, Independent_Contractors, Life_Safety_Requirements, Liquor_Liability, Martial_Arts, Tanning, Tennis_Courts, Tennis_Bubble, 
                      Number_of_Pools_Range, Abuse_Molestation, Program_Code, SUM(Ult_LDF_Capped_Incurred_Loss_ALAE_with_SS_wo_cat) 
                      AS Ult_LDF_Capped_Incurred_Loss_ALAE_with_SS_wo_cat, SUM(Ult_LDF_Capped_Incurred_Loss_ALAE_with_SS_wo_cat) 
                      / SUM(currentearnedpremium) AS Ult_Capped_Incurred_Loss_ALAE_Ratio
...
GROUP BY Coverage_Reporting_group, Fiscal_AY_Year, Policy_count_Reporting_Coverage, Renewals_Range, New_Renew_Tail, Renew_New_Auto, 
                      ...
                      GL_Class_Health_Exercise_Club_AvgRate_Range, GL_Class_Swimming_Pools_AvgRate_Range, Written_Premium_Range, GLDX_Indicator, 
                      Athletic_Participants, Daycare, Independent_Contractors, Life_Safety_Requirements, Liquor_Liability, Martial_Arts, Tanning, Tennis_Courts, 
                      Tennis_Bubble, Number_of_Pools_Range, Abuse_Molestation, Program_Code
HAVING      (SUM(currentearnedpremium) <> 0)
' 
GO
