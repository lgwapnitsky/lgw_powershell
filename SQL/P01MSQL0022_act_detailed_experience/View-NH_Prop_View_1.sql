IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[NH_Prop_View_1]'))
DROP VIEW [dbo].[NH_Prop_View_1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[NH_Prop_View_1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.NH_Prop_View_1
AS
SELECT     Coverage_Reporting_group, Coverage_Form, Fiscal_AY_Year, SUM(currentwrittenpremium) AS currentwrittenpremium, SUM(currentearnedpremium) 
                      ...
                      Written_Premium_Range, FA_Deductible, FA_Constr_Code_Dominant_NAME, FA_Avg_Bld_Rate_rd2, FA_Avg_Bld_Rate_Range, SUM(Earned_TIV) 
                      AS Earned_TIV
FROM         dbo.NH_premium_summary
GROUP BY Coverage_Reporting_group, Coverage_Form, Fiscal_AY_Year, New_Renew_Tail, Renew_New_Auto, Written_Premium_Range, FA_Deductible, 
                      FA_Constr_Code_Dominant_NAME, FA_Avg_Bld_Rate_rd2, FA_Avg_Bld_Rate_Range
' 
GO
