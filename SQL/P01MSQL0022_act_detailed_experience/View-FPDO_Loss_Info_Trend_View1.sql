IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[FPDO_Loss_Info_Trend_View1]'))
DROP VIEW [dbo].[FPDO_Loss_Info_Trend_View1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[FPDO_Loss_Info_Trend_View1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.FPDO_Loss_Info_Trend_View1
AS
...
FROM         dbo.FPDO_Loss_Detail_Trend
